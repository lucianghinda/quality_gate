  ...
  module Installation # rubocop:disable Metrics/ModuleLength
    ...
    module DirectoryOperations # rubocop:disable Metrics/ModuleLength
      ...
      def fiddle_feature_paths
        specification = bundled_fiddle_specification
        root = bundled_fiddle_root(specification)
        extension_root = bundled_fiddle_extension_root(specification)
        {
          "fiddle" => File.join(root, "fiddle.rb"),
          "fiddle.so" => File.join(extension_root, "fiddle.#{RbConfig::CONFIG.fetch("DLEXT")}"),
      ...
      def bundled_fiddle_specification
        pattern = File.join(
          RbConfig::CONFIG.fetch("prefix"),
          "lib/ruby/gems/**/specifications/fiddle-*.gemspec"
        )
        Dir[pattern].filter_map { Gem::Specification.load(_1) }.max_by(&:version) ||
          raise(Unsupported, "fiddle support is unavailable")
      end

      def bundled_fiddle_root(specification)
        root = File.join(specification.full_gem_path, "lib")
        return root if File.file?(File.join(root, "fiddle.rb"))
      ...
      def bundled_fiddle_extension_root(specification)
        ...
        raise Unsupported, "fiddle native extension is unavailable"
      end
    end
